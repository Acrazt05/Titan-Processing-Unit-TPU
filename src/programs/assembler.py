...
def assemble_file(input_file, output_mode="bin"):
    ...
    with open(input_file, "r") as f:
        for line in f:
            ...
            if output_mode == "hex":
                ...
                output_lines.append(f"{byte1} //{code} {comment}".strip())
                output_lines.append(byte2)
    ...
    #output_file = input_file.replace(".tpu", ".hex")
    output_file = "program.hex"
   
    # Fill remaining memory with 00
    MEM_SIZE = 65536
    while len(output_lines) < MEM_SIZE:
        output_lines.append("00")
